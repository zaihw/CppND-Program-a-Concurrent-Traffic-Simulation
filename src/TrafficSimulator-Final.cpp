...
void createTrafficObjects_NYC(
  ...
  // add vehicles to streets
  // nVehicles should < min(nIntersections, nStreets)
  if (nVehicles < (nIntersections > nStreets ? nStreets : nIntersections)) {
    for (size_t nv = 0; nv < nVehicles; nv++) {
      vehicles.push_back(std::make_shared<Vehicle>());
      vehicles.at(nv)->setCurrentStreet(streets.at(nv));
      vehicles.at(nv)->setCurrentDestination(intersections.at(nv));
        ...
        std::cout << "# of vehicles exceeded max allowed number, reduce to no more than: " << nIntersections > nStreets ? nStreets : nIntersections) << endl;
  }
